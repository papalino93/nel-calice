...
/**
 * Titoli, domande e dispense di una lezione del catalogo — il pannello di
 * modifica vero e proprio, senza il proprio guscio di pagina (niente
 * `AdminShell`): lo stesso componente vive dentro `/relatore/catalogo/[id]`
 * a schermo intero (§ link diretti da "Domande" nella pagina del corso) e
 * incorporato come colonna destra del master-detail in `/relatore/catalogo`.
 */
  ...
  return (
    <div>
      {backHref && (
        <Link
          href={backHref}
          className="press mb-4 inline-flex items-center gap-1.5 text-sm text-cream/55 underline underline-offset-4 hover:text-cream lg:hidden"
        >
          ← Catalogo
        </Link>
      )}

      <h2 className="mb-4 font-serif text-2xl text-cream">{lesson.titleIt}</h2>
